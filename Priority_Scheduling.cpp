...
{ 
	...
	scanf("%d",&n); 
	int at[n],bt[n],ct[n+1],wt[n],tat[n],p[n],st[n+1],gc[n],pri[n];
	...
	for(i=0;i<n;i++)
    {
        ...
        scanf("%d",&bt[i]);
        printf("Enter the priotity of the above process  : ");
        scanf("%d",&pri[i]);
        run+=bt[i];
    }
	...
	outtextxy(210,30,"For Priority Algorithm ");
    ...
    for(i=0;i<n;i++)
    {
        printf(" %2d   %2d   %2d   %2d   %2d   %2d   %2d  \n",p[i],at[i],gc[i],ct[i],tat[i],wt[i],pri[i]);
		aw = aw + wt[i];   
    }
    aw = aw/n;
